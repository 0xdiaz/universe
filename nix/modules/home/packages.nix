...
{
  # Lets a host opt out of packages that don't build there yet (e.g. rtk's
  # cargo vendoring being blocked by crates.io in some environments).
  options.universeHostPackages.excludeRtk = lib.mkEnableOption "exclude rtk from home.packages";
  ...
  config = {
    ...
    home.packages =
      ...
      ++ lib.optional (!config.universeHostPackages.excludeRtk) rtk;
  };
}
